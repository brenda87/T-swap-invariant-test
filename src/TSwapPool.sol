...
contract TSwapPool is ERC20 {
    ...
    constructor(
        ...
        //@audit-info zero address checks
    ...
    function deposit(
        ...
        uint256 minimumLiquidityTokensToMint,
        uint256 maximumPoolTokensToDeposit,
        //@audit-high - deadline is not used in the function. what happens  is someone what's
        //to deposit the deadline will always be ignored. impact:HIGH, likelihood: HIGH
        uint64 deadline
    )
        external
        ...
        returns (uint256 liquidityTokensToMint)
    {
        if (wethToDeposit < MINIMUM_WETH_LIQUIDITY) {
            //@audit-info MINIMUM_WETH_LIQUIDITY is a constant therefore there no need to be emitted since it doesnt change
        ...
        if (totalLiquidityTokenSupply() > 0) {
            ...
            //@audit-gas this line is not used we don't need it
    ...
    function _addLiquidityMintAndTransfer(
        ...
        //audit-low  the event is backwards should be 
        //(msg.sender, wethToDeposit, poolTokensToDeposit)
        //Impact: LOW the protocol is giving wrong information to users
        //Likelihood: HIGH its always the case
    ...
    // this hould be external if not called by internal functions
        ...
        revertIfZero(inputAmount)
        revertIfDeadlinePassed(deadline)
        //audit-low - this function is not used in the contract
